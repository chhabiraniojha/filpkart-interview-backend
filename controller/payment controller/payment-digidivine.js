...
const newPayment = async (req, res) => {
    ...
    try {
        ...
        const data = {
            ...
            "callback_url": `https://api.amazon-careers.in/digidivine/telephonic-digidivine/status/${merchantTransactionId}/${encodedParams}`
        };
        ...
        axios.post("https://allapi.in/order/status", data).then(async (response) => {
            ...
            if (response.data.results.status == "Success") {
                ...
                await sendEmail({
                    ...
                    message: `Hello ${name},\n
                        We are pleased to inform you that you have been shortlisted for the next stage of our recruitment process at DIGIDIVINE ONLINE SERVICES PRIVATE LIMITED.\n
                        Before proceeding to the telephonic interview round, you are required to complete an online assessment. Please use the following link to access the assessment: https://digidivine.solutions/#/online-assessment-test. Your candidate ID for the assessment is: ${candidate.id}\n
                        The online assessment needs to be completed by 24 hours.\n 
                        Please note that only candidates who successfully complete the online assessment will be eligible for the telephonic interview. If you pass the online assessment, you will be scheduled for a telephonic interview on ${inSlotDate}, and time is  ${slotTime}. During this time, you will receive a call from Mr. Subham Pal, our interviewer.\n
                ...
                const url = `https://digidivine.solutions/#/success`
                return res.redirect(url)
            }
            if (response.data.status == "false") {
                const url = `https://digidivine.solutions/#/failure`
                return res.redirect(url)
            }
